        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < allResult.Length; i++)
            {
                ...
                for (int j = 0; j < Result.Length; j++)
                {
                    ...
                    var res = All_fun.OccurrencesOfText(text, pattern);
                    if (res.Count == 0)
                    {
            ...
            /*
                ...
                StreamReader patternReader = new StreamReader(@"C:\Users\konda\Desktop\4-Data_Min-2019\DZ\lab1\pattern.txt", Encoding.Default);

                while (!patternReader.EndOfStream)
                {
                    var pattern = patternReader.ReadLine();
                    //
